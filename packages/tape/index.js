...
class Tape {
  ...
  dev() {
    ...
    async function onUpdate(updatedIds) {
      ...
      if (context) {
        for (let id of updatedIds) {
          ...
          if (has(context.transformedAssets, id)) {
            onChangeAsset({ env: 'development', asset: context.transformedAssets[id] })
          }
      ...
      const isLatest = !cache.has('context') || startedAt > cache.get('context').timestamp
      if (isLatest) {
        cache.set('context', {
          timestamp: startedAt,
          ...results.context
  ...
  async #compile({ env, context = {} }) {
    if (!env || !['development', 'production'].includes(env)) {
      throw new Error(`Expected env to be \`development\` or \`production\`. Received "${env}".`)
    }
    ...
    const generateAssetContext = (asset, context) => {
      return pick({
        asset,
        env,
        addDependency: ({ id, path }) => addDependency({ asset, id, path, dir: asset.dir }),
        resolveAsset: ({ id, path }) => resolveAsset({ id, path, dir: asset.dir }),
        getAssetContent: ({ id, path }) => getAssetContent({ id, path, dir: asset.dir }),
      }, ['asset', ...context])
    }
    ...
    while (dependencies.length > 0) {
       ...
       */
      dependencies = compact(uniq([...dependencies, ...graph.directDependenciesOf(asset.id)]))
    }
     ...
     *******************************/
     for (let id of graph.overallOrder()) {
      ...
      packagedAssets[id] = await this.#optimizeAsset(
        generateAssetContext(packagedAssets[id], ['resolveAsset', 'getAssetContent'])
      )
    ...
    return {
      ...
      files: mapValues(mapKeys(pickBy(packagedAssets, ({ embedded }) => !embedded), (asset, id) => {
  ...
  async #transformAsset({ asset, ...props }) {
     ...
     */
    const plugins = this.plugins.filter((plugin) => shouldRunPlugin(plugin, 'transform', asset.ext))
    for (let plugin of plugins) {
      ...
      const [transformedAsset, ...embeddedAssets] = forceArray(
        await this.#runPlugin(plugin,'transform', { asset: transformingAsset, ...props })
      )
      ...
      if (transformingAsset.ext !== asset.ext) {
        return [...(await this.transformAsset({ asset: transformingAsset, ...props })), ...generatedAssets]
      }
    }
  ...
  async #packageAsset({ asset, ...props }) {
    const plugin = this.plugins.find((plugin) => shouldRunPlugin(plugin, 'package', asset.ext))
  ...
  async #optimizeAsset({ asset, ...props }) {
    ...
    const plugins = this.plugins.filter((plugin) => shouldRunPlugin(plugin, 'optimize', asset.ext))
    for (let plugin of plugins) {
      optimizingAsset = await this.#runPlugin(plugin, 'optimize', { asset: optimizingAsset, ...props })
    }
  ...
  async #onChangeAsset({ asset, ...props }) {
    const plugins = this.plugins.filter((plugin) => shouldRunPlugin(plugin, 'onChange', asset.ext))
    for (let plugin of plugins) {
  ...
  #runPlugin(plugin, method, { env, ...props }) {
    return plugin[method]({
      env,
      cache: cacheNamespace(env === 'development' ? this.#cache : new Map(), plugin.name),
      ...props
    })
  }
}
